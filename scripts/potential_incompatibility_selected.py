"""Study the dynamics of potential incompatibilities under selection."""
...
def experiment(job):
    ...
    ancestor.environment = ENVIRONMENT
...
def main():
    ...
    for job, (ancestor, incp_intra, incp_inter) in enumerate(results):
        # Output the ancestral population
        ancestor.save_json(f'../data/potential_incompatibility/ancestor/experiment_{job+1}.json')

        # Ouput the counts of potential incompatibilities
        with open(f'../data/potential_incompatibility/selected/intra/experiment_{job+1}.csv', 'w') as fp:
            writer = csv.writer(fp)
            writer.writerows(incp_intra.reshape(shape).tolist())
        with open(f'../data/potential_incompatibility/selected/inter/experiment_{job+1}.csv', 'w') as fp:
            writer = csv.writer(fp)
