"""Plot properties of the underlying genetic pool in model versus control."""
...
def main():
    ...
    ax2.set_ylabel('Number of Inter-lineage\nPotential Incompatibilities',
                   fontsize=16)
    ...
    # Panel (b): potential inter-lineage incompatibilities
    # Load incompatibility data and plot the statistic over multiple
    # realizations
    logging.info('Panel (b) starts.')
    filepath = '../data/potential_incompatibility/'

    # Original model
    logging.info('Original model starts.')
    data = load_number_incomp(filepath + 'selected/inter/')
    data = data.loc[data['time'] <= T_MAX, :]  # Filter out data after T_MAX
    stats = data.groupby('time').apply(statistic, col='total').reset_index()
    ax2.plot(stats['time'], stats['med'], color='C0')
    ax2.fill_between(stats['time'], stats['lwr'], stats['upr'],
                     color='C0', alpha=0.2)

    # Control experiment with no selection pressure
    logging.info('Control scenario 1 starts.')
    data = load_number_incomp(filepath + 'neutral/inter/')
    data = data.loc[data['time'] <= T_MAX, :]  # Filter out data after T_MAX
    stats = data.groupby('time').apply(statistic, col='total').reset_index()
    ...
    handles = [mlines.Line2D([], [], color='dimgray'),
               ...
               mpatches.Patch(color='C1')]
    labels = ['Median', '95% Confidence Interval', 'Fixation', '100% Survival',
              'Model (Selection + Drift)', 'Control 1 (Drift)']
    ...
    fig.savefig('../figures/supplement_underlying_pool.pdf', dpi=300,
                bbox_inches='tight')
    plt.show()
